...
st.markdown(
    ...
    * 상관분석 결과 : 0.999 양의 상관관계로 :orange[물동량이 증가하면 화물처리량도 증가]함.
    * 가-1) 물동량 - 화물 처리실적 상관분석 참고 요망.


    __[가설] 화물처리물동량이 증가하면 체류시간 또한 증가할 것이다.__
    * 수집데이터 : 년간 부산항 물동량, 년간 부산항 화물처리량.
    * 상관분석 결과 : 0.493 양의 상관관계로 :orange[화물처리량이 증가하면 해외 선박/선원의 체류시간도 증가]함.
    ...
    * 부산항만공사 체인포탈에서 제공하는 '년간 부산항 입/출항 선원수'관련 데이터 기간이 2009~2021년까지의 데이터를 제공.
    ##### 결론
    * 부산항 물동량이 증가됨에 따라 화물처리량 또한 증가되고, :orange[입출항 해외 선원수 또한 함께 증가]됨.
    * 이는 __부산항만공사에서 해외 선박소속의 선원들을 대상으로한 서비스를 기획하고 시행할 수 있는 명분으로 작용할 수 있음.__
'''
)